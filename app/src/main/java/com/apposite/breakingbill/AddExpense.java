package com.apposite.breakingbill;
...
//
//        ((TextView)v.findViewById(R.id.textViewCustom)).setText(listName);
...
//                FrameLayout memberFrame = new FrameLayout(AddExpense.this);
//                memberFrame.setLayoutParams(new FrameLayout.LayoutParams(
//                        FrameLayout.LayoutParams.WRAP_CONTENT,FrameLayout.LayoutParams.WRAP_CONTENT));
//                memberFrame.setBackgroundColor(Color.WHITE);
//                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
//                    memberFrame.setElevation(4);
//                }
...
//                mGenerateBillBuilder = new AlertDialog.Builder(AddExpense.this)
//                        .setTitle(listName+" Bill")
//                        .setView(R.layout.layout_bill)
...
listMembers.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener() {
            ...
            public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long arg3) {
                final String list_name_selected = adapter.getItem(position);
                final AlertDialog alert = new AlertDialog.Builder(AddMember.this).create();
                final ListView del = new ListView(AddMember.this);

                del.setAdapter(new ArrayAdapter<String>(AddMember.this, android.R.layout.simple_list_item_1, new String[]{"Delete","Rename"}));
                alert.setView(del);
                del.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                    @Override
                    public void onItemClick(AdapterView<?> parent, View view, final int position, long id) {
                        if(position==0){
                            Query myQuery = mMembers.child(listId).orderByChild("memberName").equalTo(list_name_selected);
                            ...
                            myQuery.addListenerForSingleValueEvent(new ValueEventListener() {
                                ...
                                public void onDataChange(DataSnapshot dataSnapshot) {
                                    for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                                        snapshot.getRef().removeValue();
                                        Log.e(LOG_TAG, "Member " + list_name_selected + " removed.");
                                        updateTime();
                                    }
                                }
                                ...
                                @Override
                                public void onCancelled(DatabaseError databaseError) {
                                    Log.e(LOG_TAG, "Deletion cancelled.");
                                }
                            });
                            Toast.makeText(AddMember.this, list_name_selected + " Deleted", Toast.LENGTH_SHORT).show();
                            alert.dismiss();
                        }
                        else{
                            alert.dismiss();
                            AlertDialog.Builder alert1 = new AlertDialog.Builder(AddMember.this);
                            alert1.setMessage("Enter list name here");
                            final EditText inputText = new EditText(AddMember.this);
                            ...
                            alert1.setPositiveButton("Rename", new DialogInterface.OnClickListener() {
                                public void onClick(DialogInterface dialog, int whichButton) {
                                    ...
                                    if(!name.equals("")) {
                                        mMembers.child(listId).addListenerForSingleValueEvent(new ValueEventListener() {
                                            @Override
                                            public void onDataChange(DataSnapshot dataSnapshot) {
                                                for (DataSnapshot childSnapshot : dataSnapshot.getChildren()) {
                                                    try {
                                                        MemberStructure ms = childSnapshot.getValue(MemberStructure.class);
                                                        if (ms.getMemberName().equals(list_name_selected)) {
                                                            //childSnapshot.getRef().child("expenses").push().setValue(new ExpenseStructure(expenseCategory, 5000));
                                                            Map<String, Object> newListName = new HashMap<>();
                                                            newListName.put("memberName", name);
                                                            //newListName.put("timestampLastUpdated", System.currentTimeMillis());
                                                            childSnapshot.getRef().updateChildren(newListName);

                                                            Log.e(LOG_TAG, "Member " + list_name_selected + " renamed.");
                                                            updateTime();
                                                        }
                                                    } catch (Exception e) {
                                                        e.printStackTrace();
                                        ...
                                        Toast.makeText(AddMember.this, list_name_selected + " Renamed", Toast.LENGTH_SHORT).show();
                                    }
                                    else
                                        Toast.makeText(AddMember.this,"Enter a name",Toast.LENGTH_SHORT).show();
                                }
                            });
                            alert1.show();
                        }
